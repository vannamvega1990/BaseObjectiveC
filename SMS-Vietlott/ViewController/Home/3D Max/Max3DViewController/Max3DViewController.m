...
@interface Max3DViewController ()

@end

@implementation Max3DViewController {
    int totalPrice;
    ...
    float currentHeightViewChonViTriOm;
    int currentViTriOm;
}
...
- (void) initUIView {
    ...
    self.navigationItem.title = @"Max 3D";
    
    self.viewBoSoTaoRa.hidden = YES;
    self.heightViewBoSoTaoRa.constant = 0;
    
    currentHeightViewChonViTriOm = self.heightViewChonViTriOm.constant;
...
- (void)createKyQuayOffLine {
    NSArray *arrayStrDateOK = [Utils createKyQuayWithIdGame:idMax3D];
...
- (void)initTicketTypeListOffline {
    ...
    typeThuong.typeId = idMax3DCoBan;
    typeThuong.name = @"Cơ bản";
    typeThuong.quantitySelectNumber = 3;
    typeThuong.numberTicketofGroup = 6;
    [arrTicketTypes addObject:typeThuong];
    
    //add type 2
    TicketTypeObj *typeDaoSo = [[TicketTypeObj alloc] init];
    typeDaoSo.typeId = idMax3DDao;
    typeDaoSo.name = @"Đảo số";
    typeDaoSo.quantitySelectNumber = 3;
    typeDaoSo.numberTicketofGroup = 6;
    [arrTicketTypes addObject:typeDaoSo];
    
    //add type 3
    TicketTypeObj *typeOm = [[TicketTypeObj alloc] init];
    typeOm.typeId = idMax3DOm;
    typeOm.name = @"Ôm 1 vị trí";
    typeOm.quantitySelectNumber = 3;
    typeOm.numberTicketofGroup = 6;
    [arrTicketTypes addObject:typeOm];
...
- (void)onSelectedLoaiVe:(TicketTypeObj * _Nullable)selectedItem {
    if (selectedTicketType != selectedItem) {
        ...
        if ([selectedTicketType.typeId isEqualToString:idMax3DOm]) {
            ...
            self.heightViewChonViTriOm.constant = currentHeightViewChonViTriOm;
        }else{
            self.viewChonViTriOm.hidden = YES;
            self.heightViewChonViTriOm.constant = 0;
        }
...
- (IBAction)chonViTriOm:(UIButton *)sender {
    for (UIButton *btn in self.arrayBtnChonjViTriOm) {
        if (btn == sender) {
...
- (void)updateTotalPrice {
    ...
    if ([selectedTicketType.typeId isEqualToString:idMax3DCoBan]) {
    ...
    }else if ([selectedTicketType.typeId isEqualToString:idMax3DDao]) {
        for(int i = 0; i< arrayTickets.count; i++){
            BoSoObj *ticket = arrayTickets[i];
            if([ticket isValidNumber]){
                NSArray *arrayNumberCreate = [self createNumberM3D:ticket];
    ...
    }else if ([selectedTicketType.typeId isEqualToString:idMax3DOm]) {
        for(int i = 0; i< arrayTickets.count; i++){
            BoSoObj *ticket = arrayTickets[i];
            if([ticket isValidNumber]){
                NSArray *arrayNumberCreate = [self createNumberM3O:ticket];
                totalPrice += arrayNumberCreate.count*ticket.price;
                [arrayNumbers addObjectsFromArray:arrayNumberCreate];
                ticket.quantityNumber = (int)arrayNumberCreate.count;
                ticket.arrNumberCreate = arrayNumberCreate;
...
- (NSMutableArray *)createNumberM3D : (BoSoObj *)obj {
    ...
    for (int i=0; i<array.count; i++) {
        for (int j=0; j<array.count; j++) {
            for (int k=0; k<array.count; k++) {
                if (k!=i && k!=j && i!=j) {
                    NSString *str = [NSString stringWithFormat:@"%@%@%@",array[i],array[j],array[k]];
...
- (NSMutableArray *)createNumberM3O : (BoSoObj *)obj {
    NSMutableArray *arrayNumberCreate = [NSMutableArray array];
    for (int i=0; i<10; i++) {
        switch (currentViTriOm) {
            case 0:
            {
                NSString *str = [NSString stringWithFormat:@"%d%@%@",i,obj.arrNumber[1],obj.arrNumber[2]];
                [arrayNumberCreate addObject:str];
            }
                break;
            ...
            case 1:
            {
                NSString *str = [NSString stringWithFormat:@"%@%d%@",obj.arrNumber[0],i,obj.arrNumber[2]];
                [arrayNumberCreate addObject:str];
            }
                break;
            ...
            case 2:
            {
                NSString *str = [NSString stringWithFormat:@"%@%@%d",obj.arrNumber[0],obj.arrNumber[1],i];
                [arrayNumberCreate addObject:str];
            }
                break;
...
- (void)choseNumbers : (int) index {
    ...
    Max3DSelectNumberViewController *vc = [storyboard instantiateViewControllerWithIdentifier:@"Max3DSelectNumberViewController"];
...
- (void)saveTicketToCoreData : (BOOL) isBuy {
    if ([selectedTicketType.typeId isEqualToString:idMax3DCoBan]) {
        [self saveWithArray:arrayTickets:isBuy];
    }else if ([selectedTicketType.typeId isEqualToString:idMax3DDao]) {
        [self save3DDao:isBuy];
    }else if ([selectedTicketType.typeId isEqualToString:idMax3DOm]) {
        [self save3DOm:isBuy];
    }
}

- (long long)getTotalPriceFromArray : (NSArray *)arrayTicketSelected {
    long long total = 0;
    for (BoSoObj *obj in arrayTicketSelected) {
...
- (void)saveWithArray:(NSArray *)arrayTicketSelected : (BOOL) isBuy {
    NSManagedObjectContext *managedObjContext = [self managedObjectContext];
    ...
    [newItem setValue:[NSString stringWithFormat:@"%lld",[self getTotalPriceFromArray:arrayTicketSelected]] forKey:@"price"];
    [newItem setValue:selectedTicketType.name forKey:@"type_ticket"];
    [newItem setValue:smsCodeMax3D forKey:@"type_vietlott"];
    [newItem setValue:@"home_max3D" forKey:@"image"];
...
- (void)save3DDao : (BOOL) isBuy {
    for (BoSoObj *obj in arrayTickets) {
        if ([obj isValidNumber]) {
            NSMutableArray *arrayTicketsDao3D = [NSMutableArray array];
            NSArray *arrayNumberCreate = obj.arrNumberCreate;
            
            for(int i = 0; i < arrayNumberCreate.count; i++){
                BoSoObj *item = [[BoSoObj alloc] init];
                ...
                item.type = obj.type;
                item.isFavorited = obj.isFavorited;
                
                NSString *number = arrayNumberCreate[i];
                const char *c = [number UTF8String];
                item.arrNumber[0] = [NSString stringWithFormat:@"%c",c[0]];
                item.arrNumber[1] = [NSString stringWithFormat:@"%c",c[1]];
                item.arrNumber[2] = [NSString stringWithFormat:@"%c",c[2]];
                
                [arrayTicketsDao3D addObject:item];
            }
            
            [self saveWithArray:arrayTicketsDao3D :isBuy];
        }
    }
}

- (void)save3DOm : (BOOL) isBuy {
    for (BoSoObj *obj in arrayTickets) {
        if ([obj isValidNumber]) {
            NSArray *arrayNumberCreate = obj.arrNumberCreate;
            
            //5 số đầu thành 1 vé
            NSMutableArray *arrayTicketsOm1 = [NSMutableArray array];
            for(int i = 0; i < 5; i++){
                BoSoObj *item = [[BoSoObj alloc] init];
                ...
                [arrayTicketsOm1 addObject:item];
            }
            [self saveWithArray:arrayTicketsOm1 :isBuy];
            
            
            //5 số sau thành 1 vé
            NSMutableArray *arrayTicketsOm2 = [NSMutableArray array];
            for(int i = 5; i < 10; i++){
                BoSoObj *item = [[BoSoObj alloc] init];
                ...
                [arrayTicketsOm2 addObject:item];
            }
            [self saveWithArray:arrayTicketsOm2 :isBuy];
        }
    }
}
...
- (void)getRandomNumber : (BoSoObj *)ticket {
    ...
    if ([selectedTicketType.typeId isEqualToString:idMax3DOm]){
        ticket.arrNumber[currentViTriOm] = @"*";
    }
}
